...
const MainPage = ({ isDarkMode, toggleDarkMode }) => {
    ...
    return (_jsxs("div", { className: "w-full min-h-screen bg-white text-zinc-900 dark:bg-gradient-to-br dark:from-zinc-900 dark:via-zinc-800 dark:to-zinc-900 dark:text-white flex flex-col items-center px-4 pt-5 pb-7 overflow-x-hidden transition-colors duration-300 relative", children: [_jsx("div", { className: "w-full max-w-screen-xl flex justify-end px-4 sm:px-6 md:px-8 mb-6", children: _jsx(DarkModeToggle, { isDarkMode: isDarkMode, toggleDarkMode: toggleDarkMode }) }), _jsx("img", { src: isDarkMode ? lightLogo : darkLogo, alt: "Utility Package", className: "w-48 h-auto mb-10 drop-shadow-[0_2px_8px_rgba(0,0,0,0.4)]" }), _jsxs("div", { className: "mb-12 w-full max-w-md px-5 py-3 flex items-center gap-3 rounded-full bg-zinc-100 dark:bg-zinc-800 border border-zinc-300 dark:border-zinc-700 shadow-inner transition-colors duration-300 focus-within:ring-2 focus-within:ring-teal-500", children: [_jsx("span", { className: "text-zinc-500 dark:text-zinc-400 text-lg", children: "\uD83D\uDD0D" }), _jsx("input", { type: "text", placeholder: "\uC720\uD2F8\uB9AC\uD2F0 \uAC80\uC0C9...", value: search, onChange: (e) => setSearch(e.target.value), className: "flex-1 bg-transparent outline-none text-zinc-800 dark:text-white text-base placeholder:text-zinc-400 dark:placeholder:text-zinc-500" })] }), _jsx("div", { ref: containerRef, className: "w-full max-w-screen-xl px-4 sm:px-6 md:px-12 grid grid-cols-[repeat(auto-fit,minmax(240px,1fr))] gap-y-16 gap-x-4 place-items-center", children: filteredCards.map((card, index) => {
